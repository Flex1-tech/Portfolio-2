...
export default function AboutSection() {
  ...
  const academicInstitution = profile.academic_institution || "IFRI — Université d'Abomey-Calavi";
  const academicPeriod = profile.academic_period || '2024 – Present | 2nd Year';
      ...
      <div className="grid grid-cols-1 md:grid-cols-[55%_45%] gap-16">
        <div ref={contentRef} className="order-2 md:order-1">
          ...
          <div className="edu-animate mt-8">
            ...
            <p className="text-[15px] text-[#CFCFCF] mt-1">{academicInstitution}</p>
            <p className="font-mono text-[11px] text-[#737373] mt-1">{academicPeriod}</p>
          </div>
        </div>
        ...
        <div ref={imageRef} className="order-1 md:order-2 overflow-hidden">
          <img
            ...
            alt={`${profile.username || 'Seth N. AKPLOGAN'} portrait`}
            ...
            onMouseLeave={(e) => { (e.target as HTMLImageElement).style.filter = 'grayscale(0.3)'; }}
          />
        </div>
      </div>
    </section>
